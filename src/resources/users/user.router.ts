...
router.route('/:userId').get(async (req: RequestObject, res: ResultObject) => {
...
// // PUT
router.route('/:userId').put(async (req: RequestObject, res: ResultObject) => {
  const { userId } = await req.params;
  const { body } = req;
...
// // DELETE
router.route('/:userId').delete(async (req: RequestObject, res: ResultObject) => {
  const { userId } = await req.params;
